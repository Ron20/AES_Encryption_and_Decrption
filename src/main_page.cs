...
namespace project_AES
{
    public partial class main_page : Form
    {
        ...
        private void btn_encrypt_Click(object sender, EventArgs e)
        {
            ...
            string text_format = cb_iformat.Text;

            //input format
            if (text_format == "PlainText")
                AES_PlainText(data, key_array, key_length);
        ...
        private byte[] key_parsing(int key_length, string key)
        {
            ...
            int key_it = 0;
            for (int j = 0; j < 8*key_length; j += 2)
            {
        ...
        private void AES_PlainText(string data, byte[] key_array, int key_length)
        {
            //space padding
        ...
        private void btn_decrypt_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < data.Length / 32; ++i)
            {
                ...
                if(cb_iformat.Text == "PlainText")
                {
                    for (int x = 0; x < 4; x++)
                    {
                        for (int y = 0; y < 4; y++)
                        {
                            tb_decoutput.Text = tb_decoutput.Text + state[x, y].ToString("X2");
                        }
                    }
                }

                else if(cb_iformat.Text == "Text")
                {
                    for (int x = 0; x < 4; x++)
                    {
                        for (int y = 0; y < 4; y++)
